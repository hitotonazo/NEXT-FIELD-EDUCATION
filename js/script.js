...
function getProgress(){try{return Object.assign(defaultProgress(), JSON.parse(sessionStorage.getItem(STORAGE_KEY)) || {});}catch(e){return defaultProgress();}}
function saveProgress(state){sessionStorage.setItem(STORAGE_KEY, JSON.stringify(state));}
...
function applyAssetSources(){ document.querySelectorAll('[src]').forEach(el=>{ const raw = el.getAttribute('src'); if(raw && raw.startsWith('images/')) el.setAttribute('src', resolveAssetUrl(raw)); }); document.documentElement.style.setProperty('--noise-overlay-image', `url('${resolveAssetUrl('anomaly/img_noise_overlay_1600x900.png')}')`); }
function applyGlobalState(){ const p=getProgress(); if(p.anomalies && p.anomalies.graduates){document.body.classList.add('stage1-unlocked');} if(p.count>=3){document.body.classList.add('anomaly-stage');} if(p.count>=3){document.querySelectorAll('[data-stage-copy]').forEach(el=>{ if(el.dataset.stageCopyChanged) el.innerHTML=el.dataset.stageCopyChanged;}); document.title = document.body.dataset.dynamicTitle || document.title; }}
...
function initDebugAlterButton(){
  document.querySelectorAll('.debug-alter-btn').forEach(btn=>{
    if(btn.dataset.bound === "1") return;
    btn.dataset.bound = "1";
    btn.addEventListener('click', ()=>{
      btn.classList.add('is-waiting');
      const originalText = btn.textContent || 'DEBUG 5s';
      btn.textContent = '5s...';
      setTimeout(()=>{
